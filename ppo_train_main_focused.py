...
# [변경] MainTrainingEnv 사용
from env.main_train_env import MainTrainingEnv
from env.env import make_map_30

# [변경] 병렬 학습용 모듈 임포트
from rl.ppo import PPOConfig
from rl.ppo_multi import PPOTrainerMulti
from rl.vec_env import SubprocVecEnv, CloudpickleWrapper
...
def curriculum(iter_num: int, total_iters: int) -> Tuple[int, Dict[str, float], float]:
    ...
    total_iters가 20k가 아닐 때도 동일 경계(3k/6k/9k/12k)를 우선 사용.
    """
    # 경계(20k 기준). total_iters가 더 작아도 아래 구간 자체는 유지.
    t1, t2, t3, t4 = 3000, 6000, 9000, 12000
...
# ---------------------------------------- #
def make_env_fn(grid, wps, seed, focused_prob):
    def _init():
        env = MainTrainingEnv(grid=grid, waypoints=wps, seed=seed, cell_size_m=0.20, focused_training_prob=focused_prob)
        return env
    return _init

def main():
    parser = argparse.ArgumentParser(description="Train PPO with curriculum + periodic random map regeneration")
    parser.add_argument("--total-iters", type=int, default=20000, help="Total training iterations")
    ...
    # [추가] 집중 훈련 확률
    parser.add_argument("--focused-prob", type=float, default=0.3, help="Probability of focused avoidance training scenario (0.0 ~ 1.0)")
    # [추가] 병렬 환경 개수
    parser.add_argument("--num-envs", type=int, default=4, help="Number of parallel environments")
    args = parser.parse_args()

    # 멀티프로세싱 시작 방식 설정 (macOS/Linux 호환성)
    if args.num_envs > 1:
    ...
    if os.path.exists(grid_path) and os.path.exists(wps_path):
        ...
        map_seed = int(rng.integers(1, 1_000_000_000)) # 초기 시드
        print(f"[ENV] Loaded grid/waypoints from files.")
    else:
        map_seed = int(rng.integers(1, 1_000_000_000))
        grid, wps, _ = make_map_30(seed=map_seed, size=args.map_size)
        print(f"[ENV] Initialized by generating first map (seed={map_seed})")
        if args.save_maps:
    ...
    # 2. 병렬 환경 생성
    # 초기 시드 + i 로 각 환경마다 다른 시드 부여
    env_fns = [make_env_fn(grid, wps, map_seed + i, args.focused_prob) for i in range(args.num_envs)]
    vec_env = SubprocVecEnv(env_fns)

    # 관측/행동 차원 확인 (첫 번째 환경 기준)
    # SubprocVecEnv는 observation_space 속성을 직접 노출하지 않을 수 있으므로, 
    # 더미 환경을 하나 만들어서 확인하거나, vec_env의 속성을 확인해야 함.
    # 여기서는 간단히 임시 환경 하나 만들어서 확인
    dummy_env = MainTrainingEnv(grid=grid, waypoints=wps, seed=0, cell_size_m=0.20, focused_training_prob=args.focused_prob)
    obs_dim = dummy_env.observation_space.shape[0]
    act_dim = dummy_env.action_space.n
    dummy_env.close()
    
    print(f"[ENV] obs_dim={obs_dim}, act_dim={act_dim}, num_envs={args.num_envs}")
    ...
    cfg = PPOConfig(
        obs_dim=obs_dim,
        act_dim=act_dim,
        rollout_steps=4096, # 병렬 환경이므로 전체 스텝은 num_envs * rollout_steps 가 됨 (구현에 따라 다름, 여기선 각 환경당 스텝)
        lr=2e-4,
        epochs=10,
        batch_size=512,
        clip_eps=0.2,
    ...
    for it in range(1, total_iters + 1):
        ...
        if it % regen_every == 0:
            try:
                ...
                # 새 병렬 환경 생성
                env_fns = [make_env_fn(grid_new, wps_new, map_seed + i, args.focused_prob) for i in range(args.num_envs)]
                vec_env = SubprocVecEnv(env_fns)
                ...
                print(f"[ENV] Iter {it}: regenerated map with seed={map_seed}, size={args.map_size}")
                ...
                # 실패 시 복구 로직이 복잡하므로 여기서는 로그만 남기고 진행(이전 vec_env가 닫혔으므로 치명적일 수 있음)
                # 실제로는 여기서 break 하거나 재시도 로직이 필요함.
