...
def summarize_block(info):
    ...
    line1 = f"RouteBlocked={int(rblock)} | Now={int(now)} | FutureRate={fr:.2f} | BlockRate(win)={br:.2f}"
...
def visualize_episode(env, model, device="cpu", render_interval=0.05, max_steps=1000, seed=42):
    ...
    while not (done or trunc) and step < max_steps:
        ...
        robot_color = "orange" if route_blocked else "blue"
...
def main():
    ...
    env = DynAvoidOneObjEnv(grid=grid, waypoints=wps, seed=4, cell_size_m=0.20)
    obs_dim = env.observation_space.shape[0]
    act_dim = env.action_space.n

    device = "cpu"
    model = ActorCritic(obs_dim=obs_dim, act_dim=act_dim, hidden_sizes=(128,128), feat_dim=128).to(device)

    ckpt_path = "checkpoints_dyn/saved/best_ever.pt"
